...
REACTOME_KB_CURIE_ID = kg2_util.CURIE_PREFIX_IDENTIFIERS_ORG_REGISTRY + ":reactome"
...
def get_args():
    arg_parser = argparse.ArgumentParser(
        description="reactome_mysql_to_kg_json.py: \
...
def format_edge(subject_id: str, object_id: str, predicate_label: str):
    ...
    if predicate_label == kg2_util.EDGE_LABEL_BIOLINK_SAME_AS:
        return kg2_util.make_edge_biolink(
            subject_id, object_id, predicate_label, REACTOME_KB_CURIE_ID, None
        )
    return kg2_util.make_edge(
        subject_id, object_id, relation_curie, predicate_label, REACTOME_KB_CURIE_ID
    )
...
def match_name_to_prefix(name: str):
    ...
    name_prefix_dict = {
        "ENSEMBL": kg2_util.CURIE_PREFIX_ENSEMBL,
        "DDBJ": None,
...
def match_species_to_id(species: str):
    ...
    species_dict = {
        ...
        "Severe acute respiratory syndrome coronavirus 2": NCBI_TAXON_PREFIX
        + ":2697049",
...
def match_reactome_category_to_biolink(reactome_category: str, reference_class: str):
    category_dict = {
    ...
    if (
        reactome_category == "EntityWithAccessionedSequence"
...
def get_nodes(connection, nodes_output, test):
    ...
    for result in run_sql(nodes_sql, connection):
        ...
        node_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + reactome_id
        )
        if node_id is None:
            continue
        update_date = str(update_date)
        try:
            category_label = match_reactome_category_to_biolink(
                reactome_category, reference_class
            )
        ...
        node = kg2_util.make_node(
            node_id, iri, name, category_label, update_date, REACTOME_KB_CURIE_ID
        )
...
def get_reaction_inputs_and_outputs(connection, edges_output, test):
    ...
    for input in in_results:
        subject_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + input[0]
        )
        object_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + input[1]
        )
    ...
    for out in out_results:
        subject_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + out[0]
        )
        object_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + out[1]
        )
        if subject_id is None or object_id is None:
            continue
...
def get_pathway_events(connection, edges_output, test):
    ...
    for has_event in run_sql(event_sql, connection):
        subject_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + has_event[0]
        )
        object_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + has_event[1]
        )
        if subject_id is None or object_id is None:
            continue
        predicate = "has_event"
...
def get_event_characteristics(connection, edges_output, test):
    ...
    for ev_dis in run_sql(event_to_disease_sql, connection):
        subject_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + ev_dis[0]
        )
    ...
    for result in run_sql(regulation_sql, connection):
        subject_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + result[0]
        )
        object_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + result[3]
        )
        regulated_by_class = result[2]
        regulate_node = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + result[1]
        )
        if subject_id is None or object_id is None or regulate_node is None:
            continue
        regulation_to_edge_label = {
            "PositiveRegulation": "positively_regulates",
        ...
        if publications is not None:
            ...
            for publication in publications.split(","):
                ...
                publication_author_dict[
                    PMID_PREFIX + ":" + publication
                ] = citation_string
            description = result[4].replace("et al.", "et al")
            publications_info = dict()
            for publication in publications.split(","):
                publications_info[PMID_PREFIX + ":" + publication] = {
...
def get_physical_entity_characteristics(connection, edges_output, test):
    ...
    for pe_dis in run_sql(entity_to_disease_sql, connection):
        subject_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + pe_dis[0]
        )
        if subject_id is None:
            continue
...
def get_equivalencies(connection, edges_output, test):
    ...
    for go_eq in run_sql(go_eq_sql, connection):
        go_id = "GO:" + go_eq[0]
        react_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + go_eq[1]
        )
        if react_id is None:
            continue
    ...
    for ex_ont in ex_ont_results:
        ...
        react_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + ex_ont[2]
        )
        if react_id is None:
            continue
...
def get_elements_of_complex(connection, edges_output, test):
    ...
    for com_elm in run_sql(complex_elements_sql, connection):
        complex_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + com_elm[0]
        )
        element_id = only_include_certain_species(
            kg2_util.CURIE_PREFIX_REACTOME + ":" + com_elm[1]
        )
...
def get_species(connection, edges_output, test):
    ...
    for to_species_table in to_species_tables:
        ...
        for species in run_sql(species_sql, connection):
            subject_id = only_include_certain_species(
                kg2_util.CURIE_PREFIX_REACTOME + ":" + species[0]
            )
            if subject_id is None:
                continue
            try:
...
if __name__ == "__main__":
    ...
    connection = pymysql.connect(
        read_default_file=args.mysqlConfigFile, db=args.mysqlDBName
    )
    ...
    [update_date, version_number] = list(
        run_sql("SELECT releaseDate, releaseNumber FROM _Release", connection)[0]
    )

    kp_node = kg2_util.make_node(
    ...
    kg2_util.close_kg2_jsonlines(
        nodes_info, edges_info, output_nodes_file_name, output_edges_file_name
    )
