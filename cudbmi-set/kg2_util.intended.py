...
def save_json(data, output_file_name: str, test_mode: bool = False):
    ...
    if not output_file_name.endswith(".gz"):
        ...
        temp_output_file.write(
            json.dumps(data, indent=indent_num, sort_keys=sort_keys).encode("utf-8"),
            cls=DecimalEncoder,
        )
...
def close_kg2_jsonlines(
    nodes_info: tuple,
    edges_info: tuple,
    output_nodes_file_name: str,
    output_edges_file_name: str,
):
...
def make_curies_to_uri_map(
    curies_to_uri_map_yaml_string: str, mapper_type: IDMapperType
) -> dict:
    ...
    if mapper_type == IDMapperType.CONTRACT:
        return typing.cast(
            ...
            typing.cast(
                list,
                typing.cast(
                    list, yaml_data_structure_dict["use_for_bidirectional_mapping"]
                )
        ...
        return typing.cast(
            list,
            typing.cast(list, yaml_data_structure_dict["use_for_bidirectional_mapping"])
...
def get_depths_of_ontology_terms(ontology: ontobio.ontol.Ontology, top_node_id: str):
...
def get_biolink_categories_ontology_depths(biolink_ontology: ontobio.ontol.Ontology):
    url_depths = get_depths_of_ontology_terms(
        biolink_ontology, CURIE_PREFIX_BIOLINK + ":NamedThing"
    )
    ret_depths = {
        key.replace(BASE_URL_BIOLINK_META, ""): value
        for key, value in url_depths.items()
    }
                ...
                if type(value) == str and type(stored_value) == str:
                    if value.lower() != stored_value.lower():
                        if key == "update_date":
                            ...
                            if biolink_depth_getter is not None:
                                depth_x = biolink_depth_getter(
                                    CURIE_PREFIX_BIOLINK
                                    + ":"
                                    + convert_snake_case_to_camel_case(
                                        stored_value, uppercase_first_letter=True
                                    )
                                )
                                depth_y = biolink_depth_getter(
                                    CURIE_PREFIX_BIOLINK
                                    + ":"
                                    + convert_snake_case_to_camel_case(
                                        value, uppercase_first_letter=True
                                    )
                                )
                                ...
                                if "named_thing" != value:
                                    if stored_value == "named_thing":
                                        ...
                                        log_message(
                                            message="inconsistent category_label information; keeping original category_label "
                                            + stored_value
                                            + " and discarding new category_label "
                                            + value,
                                            ontology_name=str(
                                                x.get(
                                                    "provided_by", "provided_by=UNKNOWN"
                                                )
                                            ),
                            ...
                            if biolink_depth_getter is not None:
                                ...
                                if not value.endswith("NamedThing"):
                                    if stored_value.endswith("NamedThing"):
                                        ...
                                        log_message(
                                            message="inconsistent category information; keeping original category "
                                            + stored_value
                                            + " and discarding new category "
                                            + value,
                                            ontology_name=str(
                                                x.get(
                                                    "provided_by", "provided_by=UNKNOWN"
                            ...
                            if value.replace(" ", "_") != stored_value.replace(
                                " ", "_"
                            ):
                    ...
                    if key != "synonym":
                        ...
                        if len(stored_value) > 0:
                            ...
                            first_element = set()
                        ret_dict[key] = list(first_element) + sorted(
                            filter(
                                None, list(set(value + stored_value) - first_element)
                            )
                        )
                    ...
                    ret_dict[key] = merge_two_dicts(
                        value, stored_value, biolink_depth_getter
                    )
                elif key == "deprecated" and type(value) == bool:
                    ret_dict[
                        key
                    ] = True  # special case for deprecation; True always trumps False for this property
                else:
                    log_message(
                        message="invalid type for key: " + key,
...
def convert_snake_case_to_camel_case(name: str, uppercase_first_letter: bool = False):
...
def make_node(
    ...
    if "-" in category_label:
        raise ValueError(
            "underscore character detected in category_label argument to function kg2_util.make_node: "
            + category_label
        )
    return {
        "id": id,
...
def make_edge_key(edge_dict: dict):
    return (
        ...
        + (
            ...
            if edge_dict["qualified_predicate"] is not None
            else "None"
        )
        + "---"
        + (
            edge_dict["qualified_object_aspect"]
            if edge_dict["qualified_object_aspect"] is not None
            else "None"
        )
        + "---"
        + (
            edge_dict["qualified_object_direction"]
            if edge_dict["qualified_object_direction"] is not None
            else "None"
        )
        + "---"
        + edge_dict["object"]
...
def predicate_label_to_curie(predicate_label: str, relation_curie_prefix: str):
...
def ont_children_recursive(ont_hier: ontobio.ontol.Ontology, node_name: str):
    res_set = {node_name}
...
def make_edge_biolink(
    subject_curie_id: str,
    object_curie_id: str,
    predicate_label: str,
    provided_by_curie: str,
    update_date: str,
):
    relation_curie = predicate_label_to_curie(predicate_label, CURIE_PREFIX_BIOLINK)
    rel = make_edge(
        subject_curie_id,
        object_curie_id,
        relation_curie,
...
def make_ontology_from_local_file(file_name: str, save_pickle: bool = False):
    ...
    if not os.path.isfile(file_name_with_pickle_ext) or save_pickle:
        ...
        if not file_name.endswith(".json"):
            temp_file_name = (
                tempfile.mkstemp(prefix=TEMP_FILE_PREFIX + "-")[1] + ".json"
            )
            size = os.path.getsize(file_name)
            log_message(
                message="Reading ontology file: "
                + file_name
                + "; size: "
                + "{0:.2f}".format(size / 1024)
                + " KiB",
                ontology_name=None,
            )
            cp = subprocess.run(
                ["owltools", file_name, "-o", "-f", "json", temp_file_name], check=True
            )
            ...
            if cp.stdout is not None:
                log_message(
                    message="OWL convert result: " + cp.stdout,
                    ontology_name=None,
                    output_stream=sys.stdout,
                )
            if cp.stderr is not None:
                log_message(
                    message="OWL convert result: " + cp.stderr,
                    ontology_name=None,
                    output_stream=sys.stderr,
                )
        ...
        log_message(
            message="Reading ontology JSON file: "
            + json_file
            + "; size: "
            + "{0:.2f}".format(size / 1024)
            + " KiB",
